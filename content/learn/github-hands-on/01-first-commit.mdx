...
レッスンを進めるごとにページを充実させていき、最終的に GitHub 上に公開された自分のプロフィールページが完成します。
...
初期化が終わり、`git-learn`ディレクトリに`.git`フォルダを作成した状態です。
...
このレッスンでは、ガイドで学んだ Git の基本コマンドを実際に使ってみました。

- `git init` でリポジトリを作成
- `git add` → `git commit` で変更を記録
- `git status` で各ステップの状態を確認
- `git diff` で変更内容を確認
- `git log` でコミット履歴を確認
